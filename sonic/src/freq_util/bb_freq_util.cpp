...
void freq_init() {
    ...
    if (freq_init_is_high) {
        ...
        for (i=0, len = strlen(BB_CHARACTERS); i<len; ++i) {
            ...
            unsigned int freq = (unsigned int)floor(BB_BASEFREQUENCY * pow(BB_FREQUENCY_STEP, i));
            frequencies[i] = freq;
...
int freq_to_num(unsigned int f, int *n) {
    ...
    /*
     frequencies[0] = (unsigned int)floor(BB_BASEFREQUENCY * pow(BB_FREQUENCY_STEP, 0));
     frequencies[31] = (unsigned int)floor(BB_BASEFREQUENCY * pow(BB_FREQUENCY_STEP, 31));
     
     
     if (n != NULL &&
     f >= frequencies[0]-BB_THRESHOLD*pow(BB_FREQUENCY_STEP, 0) &&
     f <= frequencies[31]+BB_THRESHOLD*pow(BB_FREQUENCY_STEP, 31)) {
     ...
     if (abs(frequencies[i] - f) <= BB_THRESHOLD*pow(BB_FREQUENCY_STEP, i)) {
...
int num_to_freq(int n, unsigned int *f) {
	...
	if (f != NULL && n>=0 && n<32) {
		...
		//*f =  (unsigned int)floor(BB_BASEFREQUENCY * pow(BB_FREQUENCY_STEP, n));
