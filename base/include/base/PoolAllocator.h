...
template <typename T,
	typename PoolType = CMemoryPool,        // 指定内存池类型
	typename Mutex = Pool::DefaultMutex,    // 指定锁类型, Detial::CMemPool 内部已经加锁，这里用 NullMutex
	typename SingletonTag = Pool::DefaultSingletonTag,
	unsigned nextSize = 32,                 // 每次分配的区块数
	unsigned maxSize = 0>
...
/// \brief 符合std:: alloctor定义的内存模式内存分配器
/// 对象内存使用CMemoryPool管理，由于是静态变量，所以程序生命周期中申请到的系统内存一直不会释放
template <typename T,
	typename PoolType,
	typename Mutex,
	typename SingletonTag,
